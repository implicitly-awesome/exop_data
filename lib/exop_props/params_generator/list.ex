defmodule ExopProps.ParamsGenerator.List do
  ...
  defp do_generate(%{inner: _} = opts, props_opts), do: StreamData.map(generator(opts, props_opts), &Enum.into(&1, []))

  defp do_generate(opts, props_opts), do: StreamData.list_of(list_item(opts, props_opts), length_opts(opts))
  ...
  defp list_item(opts, props_opts) do
    ...
    if Enum.any?(opts) do
      ParamsGenerator.resolve_opts(opts, props_opts)
    else
